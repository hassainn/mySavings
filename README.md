...
**Refresh AI Strategist** runs at 08:15 and 18:00 IST on weekdays, 10:15 IST on weekends, after successful scanner/news workflows, and on manual dispatch. It calls Gemini only when a complete plan and `GEMINI_API_KEY` are available; missing credentials, timeouts and malformed AI responses retain the deterministic plan.

When Gemini is reachable, the enrichment reasons like a senior analyst using the documented public methods of legendary traders — Minervini's SEPA (8-point Trend Template, VCP, pivot/no-chase, risk discipline), O'Neil's CANSLIM, Weinstein stage analysis, Darvas and Livermore — and, via Google Search grounding, adds per-candidate **technical + fundamental** analysis (stage, trend/VCP/RS read, earnings/sales/margin/ROE/debt findings), an educational verdict (Leading / Constructive / Watch / Extended / Avoid) and a risk note. Grounded fundamentals need a billed Gemini project; on the free tier the deterministic plan stands. The AI is stored separately from the plan and **can never change the gate, candidate list or scanner price levels**, and it is educational research, not advice or an order. The Pages workflow deploys after strategist completion, including updates committed using the Actions token.
